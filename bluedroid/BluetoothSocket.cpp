...
bool
BluetoothSocket::ConnectSocket(const nsAString& aDeviceAddress, const uint8_t aUuid[], int aChannel)
{
  MOZ_ASSERT(NS_IsMainThread());
...
bool
BluetoothSocket::ListenSocket(const uint8_t aUuid[], int aChannel)
{
  MOZ_ASSERT(NS_IsMainThread());
  NS_ENSURE_FALSE(mImpl, false);
  ...
  nsString aDesc;
  if(aUuid[3] == 0x05) {
      aDesc = NS_LITERAL_STRING("OBEX Object Push");
  } else if (aUuid[3] == 0x01) {
      aDesc = NS_LITERAL_STRING("Serial Port");
  }
