...
void
BluetoothServiceBluedroid::SendSerialPort(const nsAString& aDeviceAddress,
                                          ...
                                          BluetoothReplyRunnable* aRunnable)
{
    ...
    // has been determined when calling 'Connect()'. Nevertheless, keep
    // it for future use.
    BluetoothSppManager* spp = BluetoothSppManager::Get();
    nsAutoString errorStr;
    if (!spp || !spp->SendData(aDeviceAddress, aData)) {
      errorStr.AssignLiteral("Calling SendData() failed");
    }
