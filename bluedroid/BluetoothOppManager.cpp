...
BluetoothOppManager::ConnectInternal(const nsAString& aDeviceAddress)
{
  ...
  mSocket->ConnectSocket(aDeviceAddress, UUID_OBEX_OBJECT_PUSH, -1);
}
...
BluetoothOppManager::Listen()
{
  ...
  if (!mServerSocket->ListenSocket(UUID_OBEX_OBJECT_PUSH, BluetoothReservedChannels::CHANNEL_OPUSH)) {
